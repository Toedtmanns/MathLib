...
namespace MathLib
{
	...
	MatrixF MatrixRotate(const MatrixF& mat, const Quaternion& quat)
	{
		if (!MatrixIsSquare(mat, 4))
			throw std::invalid_argument("Matrix is not 4x4!");

		Quaternion xQuat(Float3(mat.GetNum(0, 0), mat.GetNum(0, 1), mat.GetNum(0, 2)));
		Quaternion yQuat(Float3(mat.GetNum(1, 0), mat.GetNum(1, 1), mat.GetNum(1, 2)));
		Quaternion zQuat(Float3(mat.GetNum(2, 0), mat.GetNum(2, 1), mat.GetNum(2, 2)));

		xQuat = const_cast<Quaternion&>(quat).RotateQuaternion(xQuat);
		yQuat = const_cast<Quaternion&>(quat).RotateQuaternion(yQuat);
		zQuat = const_cast<Quaternion&>(quat).RotateQuaternion(zQuat);
